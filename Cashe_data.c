...
int cpuid_caches(size_t* data_caches) {
    ...
    for (i = 0; i < 32; i++) {
        ...
        // Variables to hold the contents
        ...
        size_t cache_total_size = static_cast<size_t>(cache_ways_of_associativity) * cache_physical_line_partitions * cache_coherency_line_size * cache_sets;
...
int test_cache(size_t attempts, size_t lower_cache_size, int* latencies, size_t max_latency) {
    ...
    while (attempts--) {
        ...
        int32_t cycles_used = 0, edx = 0, temp1 = 0, temp2 = 0;
        asm(
            "mfence\n\t"        // memory fence
            "rdtsc\n\t"         // get cpu cycle count
            "mov %%edx, %2\n\t"
